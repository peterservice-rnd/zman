...
class LdapAuthenticationViaUserSearchTest {

    @Test
    fun positiveAuthentication() {
        val requestBuilder = formLogin().user(testUserName).password(testUserPassword)
    ...
    fun failWithNotAuthenticatedWithWrongUser() {
        val requestBuilder = formLogin().user("this_user_doesnt_exist").password("1111")
        mockMvc.perform(requestBuilder)
    ...
    fun failWithNotAuthenticatedWithWrongPassword() {
        val requestBuilder = formLogin().user(testUserName).password(testUserPassword + "1111")
        mockMvc.perform(requestBuilder)
                .andExpect(unauthenticated())
    }
